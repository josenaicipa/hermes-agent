...
def _build_healthy_legacy_db(db_path: Path, *, n_pairs: int = 5) -> str:
    """One session with ``n_pairs`` pairs on a genuine LEGACY (pre-v23)
    inline-FTS schema: single-column ``messages_fts``, ``schema_version=10``.

    ``_rebuild_legacy_fts_indexes`` (DELETE + explicit ``INSERT ... SELECT``)
    only applies to this self-contained inline shape. v23's ``messages_fts``
    is external-content (``content='messages'``) — a bare ``DELETE FROM
    messages_fts`` does not clear an external-content FTS5 index the way it
    does an inline one, and repopulation there uses the special ``'rebuild'``
    command instead (see ``_rebuild_fts_indexes``). ``SessionDB`` opening
    never auto-migrates a legacy DB (opt-in via ``hermes db optimize`` only),
    so this schema survives the ``SessionDB(db_path=db_path)`` open below
    unchanged — mirrors the proven pattern in
    ``tests.test_hermes_state.TestFtsMigration.
    ...
    for i in range(n_pairs):
        conn.execute(
            ...
            "VALUES (?, ?, 'user', ?)",
            (sid, time.time(), f"historical pizza message {i}"),
        )
        conn.execute(
            "INSERT INTO messages (session_id, timestamp, role, content) "
            "VALUES (?, ?, 'assistant', ?)",
            (sid, time.time(), f"assistant reply about pasta {i}"),
        )
    conn.commit()
    conn.close()
    return sid
...
def test_rebuild_fts_rolls_back_and_preserves_history_on_failure(tmp_path):
    ...
    _build_healthy_legacy_db(db_path, n_pairs=5)
    ...
    try:
        ...
        with pytest.raises(sqlite3.OperationalError):
            SessionDB._rebuild_legacy_fts_indexes(proxy, include_trigram=False)
...
def test_rebuild_fts_success_reindexes_every_row(tmp_path):
    ...
    try:
        ...
        SessionDB._rebuild_legacy_fts_indexes(db._conn.cursor(), include_trigram=False)
