...
CHANNEL = "1414140000000000001"
THREAD = "1414140000000000002"
...
_FROZEN_CHANNEL = "1524886484539412510"
_FROZEN_THREAD = "1551631620098891788"
...
def test_frozen_contract_record_key():
    assert record_key(
        channel_id=_FROZEN_CHANNEL, thread_id=_FROZEN_THREAD, body=_FROZEN_BODY
    ) == "29687501c8bc4c8ad4e03d2247b00904e622f02c02964e0bcd97f404a6c94987"


def test_frozen_contract_signature():
    assert signature(
        _FROZEN_KEY,
        channel_id=_FROZEN_CHANNEL,
        thread_id=_FROZEN_THREAD,
    ...
    ) == "edde43399a3d8020ea71e250ee5aeb29f35e802e7225617e2c575f168faa98ea"
...
def test_reader_without_key_admits_nothing(tmp_path):
    ...
    assert box.consume(channel_id=CHANNEL, thread_id=THREAD, body=_body(text)) is None
...
def test_valid_record_is_consumed_once(inbox):
    ...
    claimed = inbox.consume(channel_id=CHANNEL, thread_id=THREAD, body=_body(text))
    assert claimed is not None
    ...
    assert inbox.consume(channel_id=CHANNEL, thread_id=THREAD, body=_body(text)) is None
...
def test_peek_does_not_spend_the_record(inbox):
    ...
    assert inbox.peek(channel_id=CHANNEL, thread_id=THREAD, body=_body(text)) is True
    assert inbox.consume(channel_id=CHANNEL, thread_id=THREAD, body=_body(text)) is not None


def test_missing_record_is_not_ingested(inbox):
    assert inbox.peek(channel_id=CHANNEL, thread_id=THREAD, body=_body("nadie escribió esto")) is False
...
def test_record_of_another_key_is_rejected(tmp_path):
    ...
    assert victim.consume(channel_id=CHANNEL, thread_id=THREAD, body=_body(text)) is None
...
def test_record_cannot_be_replayed_into_another_channel(inbox):
    text = "hola"
    name = inbox.write(_record(text), body=_body(text))
    otro_canal = "1414140000000000777"
    # Copiar el registro bajo la dirección del otro canal no lo hace válido:
    # canal y hilo están dentro de la firma.
    (inbox.directory / f"{record_key(channel_id=otro_canal, thread_id='', body=_body(text))}.json").write_bytes(
        (inbox.directory / name).read_bytes()
    )

    assert inbox.consume(channel_id=otro_canal, thread_id="", body=_body(text)) is None
...
def test_record_cannot_vouch_for_another_body(inbox):
    ...
    assert inbox.consume(channel_id=CHANNEL, thread_id=THREAD, body=_body("otra cosa")) is None
...
def test_declared_message_id_must_match_the_live_message(inbox):
    ...
    assert inbox.consume(
        channel_id=CHANNEL, thread_id=THREAD, body=_body(text), message_id="111111111111111111"
    ) is None
...
def test_record_key_is_a_bare_hex_name(inbox):
    ...
    name = record_key(channel_id=CHANNEL, thread_id="../../etc", body="x")
...
def test_concurrent_claims_yield_exactly_one_winner(inbox):
    ...
    inbox.write(_record(text), body=_body(text))
    ganadores = [
        inbox.consume(channel_id=CHANNEL, thread_id=THREAD, body=_body(text), now=time.time())
        for _ in range(5)
    ]
