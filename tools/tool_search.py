...
class ToolSearchConfig:
    ...
    def from_raw(cls, raw: Any) -> "ToolSearchConfig":
        ...
        visible_raw = raw.get("always_visible", [])
        if isinstance(visible_raw, str):
            visible_raw = [visible_raw]
        if not isinstance(visible_raw, (list, tuple, set, frozenset)):
            visible_raw = []
        return cls(
            ...
            always_visible=frozenset(str(n).strip() for n in visible_raw if str(n or "").strip()),
...
def is_deferrable_tool_name(name: str, defer_tools: Optional[frozenset] = None,
                          always_visible: frozenset[str] | None = None) -> bool:
...
def classify_tools(tool_defs: List[Dict[str, Any]], defer_tools: Optional[frozenset] = None,
    ...
    for td, name in zip(tool_defs, _tool_def_names(tool_defs)):
        if name not in BRIDGE_TOOL_NAMES:
            (deferrable if is_deferrable_tool_name(name, defer_tools, always_visible) else visible).append(td)
...
def assemble_tool_defs(tool_defs: List[Dict[str, Any]], *, context_length: Optional[int] = None,
    ...
    incoming = [td for td, name in zip(tool_defs, _tool_def_names(tool_defs))
                if name not in BRIDGE_TOOL_NAMES]
    visible, deferrable = classify_tools(incoming, config.effective_defer_tools, config.always_visible)
...
def dispatch_tool_describe(args: Dict[str, Any], *, current_tool_defs: List[Dict[str, Any]],
    ...
    for name in names:
        ...
        if fn is not None:
            ...
            name, load_config_readonly().effective_defer_tools, load_config_readonly().always_visible):
...
def scoped_deferrable_names(tool_defs: List[Dict[str, Any]]) -> frozenset[str]:
    ...
    return frozenset(n for n in _tool_def_names(tool_defs)
                     if n and is_deferrable_tool_name(n, defer_tools, config.always_visible))
...
def resolve_underlying_call(args: Dict[str, Any]) -> Tuple[Optional[str], Dict[str, Any], Optional[str]]:
    ...
    * the trajectory recorder.
    ...
    if not is_deferrable_tool_name(name, load_config_readonly().effective_defer_tools, load_config_readonly().always_visible):
        return None, {}, (
