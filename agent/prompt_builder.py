...
def build_skills_system_prompt(
    ...
    if not skills_by_category:
        ...
        result = (
            ...
            "about this channel's context, scope, memories, or citations, answer from that "
            "AGENT_CONTEXT first. Do NOT load broad workspace/routing skills (e.g. "
            "`jose-workspace-context`) as the primary source for that question. Only load "
            "them when the user explicitly asks for global routing, when Context Fabric "
            "lists them as required, or when AGENT_CONTEXT is absent or empty — and in "
            "that case label them as fallback/auxiliary. Any skill or tool consulted after "
            "AGENT_CONTEXT must be labeled as an additional source, never as the Context "
            "Fabric source.\n"
